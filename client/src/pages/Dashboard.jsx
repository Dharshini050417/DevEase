...
export default function DeployedProjectsDashboard() {
  ...
  useEffect(() => {
    ...
    //   try {
    //     const response = await fetch(`http://localhost:5000/api/project/user/${user.id}`);
  ...
  useEffect(() => {
    const fetchDeploymentDetails = async () => {
      ...
      //   try {
      //     const response = await fetch(`http://localhost:5000/api/pipeline/deployment/${project.id}`);
      //     const data = await response.json();
            ...
            return (
              <Card key={project.id} className="w-[48%] bg-gray-900 shadow-lg">
                <CardContent className="flex flex-col space-y-4 p-6">
                  ...
                  <span className={`px-3 py-1 text-sm text-white ${getStatusBadge(project.status)}`}>
                    {project.status}
                  </span>

                  {details && !details.error ? (
                    <>
                      <p className="text-gray-400 text-sm">Instance IP: {details.instance_ip}</p>
                      <p className="text-gray-400 text-sm">Status: {details.status}</p>
                      <Button
                    ...
                    <p className="text-red-400 text-sm">{details?.error || "Loading deployment details..."}</p>
                  )}

                  <div className="flex space-x-3 mt-4">
                    <Button className="!bg-white !text-black text-sm hover:bg-grey px-4 py-2">View Logs</Button>
                    <Button className="!bg-white !text-black text-sm px-4 py-2">Restart</Button>
                    <Button className="!bg-white !text-black text-sm hover:bg-grey px-4 py-2">Delete</Button>
                  </div>
                </CardContent>
              </Card>
            );
